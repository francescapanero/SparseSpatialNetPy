...
def GGPrnd(t, sigma, c, T):
    ...
    if sigma < -1e-8:
        ...
        T = 0
        return N
    # Use a truncated Pareto on [T,a] and on (a, infinity) use truncated exponential (see later)
    if sigma > 0:
    ...
    log_N2 = np.log(a + np.random.exponential(1 / c, Njumps))  # Sample from truncated exponential
